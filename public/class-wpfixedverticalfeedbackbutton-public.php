<?php
	...
	class Wpfixedverticalfeedbackbutton_Public {
		...
		public function enqueue_styles() {
			...
			wp_register_style('wpfixedverticalfeedbackbutton-public', plugin_dir_url( __FILE__ ) . '../assets/css/wpfixedverticalfeedbackbutton-public.css', array(), $this->version, 'all' );
			wp_enqueue_style( 'wpfixedverticalfeedbackbutton-public' );
		...
		public function enqueue_scripts() {
			wp_register_script( 'wpfixedverticalfeedbackbutton-public', plugin_dir_url( __FILE__ ) . '../assets/js/wpfixedverticalfeedbackbutton-public.js', array( 'jquery' ), $this->version, true );
			wp_enqueue_script( 'wpfixedverticalfeedbackbutton-public');
		...
		public function add_button_html() {
			...
			if ( $the_query->have_posts() ) {
				...
				while ( $the_query->have_posts() ) {
					...
					$btext_cust_height  = isset( $fieldValues['btext_height'] ) ? absint( $fieldValues['btext_height'] ) : ''; //button custom image height in px
					$btext_cust_width   = isset( $fieldValues['btext_width'] ) ? absint( $fieldValues['btext_width'] ) : ''; //button custom image width in px
					...
					$imageOrText                   = '';
					$fvfeedbackbutton_source_class = 'fvfeedbackbutton_source';
					if ( $btext == 'custom_img' ) {
						if ( $btext_cust_img != 'custom_img' ) {
							$imageOrText                   = '<img src="' . $btext_cust_img . '" style="height:' . $btext_cust_height . '" alt="btnimage" />';
							$fvfeedbackbutton_source_class = 'fvfeedbackbutton_source_customimg';
						}
					} else if ( $btext == 'custom_text' ) {
						//$imageOrText = strrev($btext_cust_text);
						$imageOrText                   = $btext_cust_text;
						$fvfeedbackbutton_source_class = 'fvfeedbackbutton_source_customtext';
					} else {
						$imageOrText                   = '<img src="' . plugins_url( '../assets/images/' . $btext, __FILE__ ) . '" alt="' . $btext . '" />';
						$fvfeedbackbutton_source_class = 'fvfeedbackbutton_source_img';
					}
					...
					if ( $showtype == 1 && in_array( $gobal_post_id, $postlist ) || $showtype == 0 && ! in_array( $gobal_post_id, $postlist ) || empty( $postlist ) ) {
						...
						$anchorExtra       = apply_filters( 'wpfvfbtn_button_html_anchorextra', $anchorExtra, $loop_post_id, $fieldValues );
						...
						if ( $btext == 'custom_img' ) {
							...
							$width   = $btext_cust_width;
							//var_dump($width);

							//$height   = intval( $height );
							if ( $height == 0 ) {
								$height_style = ' height:120px; ';
							} else {
								$height       += 10;
							...
							$imageurl = WP_PLUGIN_URL . '/wpfixedverticalfeedbackbutton/assets/images/' . $buttontext;
						...
						if ( $extra_style != '' ) {
							echo '<style type="text/css" media="screen">' . $extra_style . '</style>';
						}

						$anchor_output = '<div class="fvfeedbackbutton ' . $class . ' " id="fvfeedbackbutton' . $loop_post_id . '"><a '.$anchorExtra.' class="' . $anchorClass . '  ' . $fvfeedbackbutton_source_class . '"  href="' . $link . '" data-count = "' . $loop_post_id . '" data-show-form = "' . $cbwpfvb_show_form . '" title="' . $title . '" target="' . $open . '"><span>' . $imageOrText . '</span></a></div>';
